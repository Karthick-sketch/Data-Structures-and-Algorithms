...
public:
  ...
  T operator [] (int index_number) {
    return get(index_number);
  }
  ...
  void insert(int pos, T val) {
    if (pos < 0) {
      exitProgram("Invalid index number");
    } else if (pos == 0) {
      unshift(val);
    } else if (pos >= length()) {
      push(val);
  ...
  void removeAt(int pos) {
    if (pos >= 0 && pos < length()) {
      ...
      if (current_node == head) {
        head = current_node->getLink();
      } else if (current_node == tail) {
        tail = previous_node;
        tail->setLink(NULL);
      } else {
        previous_node->setLink(current_node->getLink());
      }
      delete current_node;
    }
  }
